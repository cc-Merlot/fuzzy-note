<!-- 用户资料页（自有 fork，覆盖 uni_modules/uni-id-pages/pages/userinfo/userinfo）
     唯一改动：默认头像走 CDN（commonImage.defAvatar） -->
<template>
	<view class="uni-content">
		...
		<view class="avatar">
			<view class="box">
				<view class="inner">
					...
					<image v-else :src="commonImage?.defAvatar" mode="aspectFill"></image>
		...
		<uni-list>
			...
			<uni-list-item class="item" @click="copyID" title="ID" :rightText="userInfo._id" link></uni-list-item>
...
<script>
	...
	export default {
		...
		data() {
			return {
				commonImage, // Options API 不会自动暴露 import，需显式放进 data
				univerifyStyle: {
					authButton: {
