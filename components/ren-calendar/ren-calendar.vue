<template>
    <view class="calendar-wrapper">
        <view class="header" v-if="headerBar">
            <view class="pre" @click="changeMonth('pre')"><uni-icons type="left" size="20"></uni-icons></view>
            <view>{{ y + '年' + formatNum(m) + '月' }}</view>
            <view class="next" @click="changeMonth('next')"><uni-icons type="right" size="20"></uni-icons></view>
        </view>
        <view class="week">
...
export default {
    ...
    methods: {
        ...
        changeMonth(type) {
            ...
            this.dates = this.monthDay(this.y, this.m);
            this.$emit('onChangeMonth', { year: this.y, month: this.m });
        }
    }
};
</script>
